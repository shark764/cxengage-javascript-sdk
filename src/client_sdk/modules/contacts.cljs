...
(defn ^:private contact-request
  ...
  (let [request-map {:url url
                     ...
                     :resp-chan resp}]
    (when body
      (assoc request-map :body body))
    (when token
      (assoc request-map :token token))
...
(defn delete-contact
  ...
  (let [{:keys [token contact-id tenant-id resp-chan attributes]} message
        resp (a/promise-chan)
        request-map {:url (u/api-url (:env @module-state) (str "/tenants/" tenant-id "/contacts/" contact-id))
                     :method :delete
                     :token token
                     :resp-chan resp}]
    (u/api-request request-map)
    (go
      (when (= (:status (a/<! resp)) 201)
        (a/>! resp-chan true)))))
...
(defn create-attribute
  [message]
  (let [{:keys [token tenant-id resp-chan attribute]} message
        resp (a/promise-chan)
        url (u/api-url (:env @module-state) (str "/tenants/" tenant-id "/contacts/attributes"))
        body attribute
        method :post]
